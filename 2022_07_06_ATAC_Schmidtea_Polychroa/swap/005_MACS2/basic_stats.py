...
df1 = pd.read_csv ("chr_SP_1_ds_peaks.narrowPeak", sep = "\t", names=colnames)
df1['length']=df1['chromEnd']-df1['chromStart']
df1['replicate'] = "SP_1"
df1_row = df1.shape[0]
print ("SP_1 has " + str(df1_row) + " peaks")

df2 = pd.read_csv ("chr_SP_2_ds_peaks.narrowPeak", sep = "\t", names=colnames)
df2['length']=df2['chromEnd']-df2['chromStart']
df2['replicate']="SP_2"
df2_row = df2.shape[0]
print ("SP_2 has " + str(df2_row) + " peaks")

df3 = pd.read_csv ("chr_SP_3_ds_peaks.narrowPeak", sep = "\t", names=colnames)
df3['length']=df3['chromEnd']-df3['chromStart']
df3['replicate'] = "SP_3"
df3_row = df3.shape[0]
print ("SP_3 has " + str(df3_row) + " peaks")
...
plt.savefig("chr_SP_peak_length.svg",format='svg')
